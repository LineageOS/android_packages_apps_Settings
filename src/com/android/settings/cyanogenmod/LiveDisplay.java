...
public class LiveDisplay extends SettingsPreferenceFragment implements
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mLiveDisplay.setEntries(entries);

        mValues = getResources().getStringArray(
                com.android.internal.R.array.live_display_values);
        mLiveDisplay.setEntryValues(mValues);

        // Remove outdoor mode if there is no support
        if (!SunlightEnhancement.isSupported()) {
            int idx = ArrayUtils.indexOf(mValues, MODE_OUTDOOR);
            String[] entriesTemp = new String[entries.length - 1];
            String[] valuesTemp = new String[mValues.length - 1];
            int j = 0;
            for (int i = 0; i < entries.length - 1; i++) {
                if (i == idx) {
                    continue;
                }
                entriesTemp[j] = entries[i];
                valuesTemp[j] = mValues[j];
            }
            entries = entriesTemp;
            mValues = valuesTemp;
        }
        ...
        int index = ArrayUtils.indexOf(mValues, String.valueOf(displayMode));
    ...
    public boolean onPreferenceChange(Preference preference, Object objValue) {
        if (preference == mLiveDisplay) {
            ...
            int index = ArrayUtils.indexOf(mValues, (String)objValue);
            mLiveDisplay.setSummary(getResources().getStringArray(
                    com.android.internal.R.array.live_display_summaries)[index]);
        }
        return true;
