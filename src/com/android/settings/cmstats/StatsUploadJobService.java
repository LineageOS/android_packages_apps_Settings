...
public class StatsUploadJobService extends JobService {
    ...
    private class StatsUploadTask extends AsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) {
            ...
            if (success) {
                synchronized (mCurrentJobs) {
                    // if it was removed in onStopJob() because criteria was no longer met
                    // then we need to make sure we fail out here to reschedule the job
                    success = mCurrentJobs.remove(mJobParams) != null;
                }
                if (success) {
