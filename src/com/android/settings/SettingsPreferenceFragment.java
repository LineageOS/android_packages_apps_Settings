...
public class SettingsPreferenceFragment extends PreferenceFragment implements DialogCreatable {
    ...
    private List<PreferenceNodeInfo> findKeysToRemove(int xmlResId) {
        ...
        try {
            ...
            final AttributeSet preferenceScreenAttrs = Xml.asAttributeSet(parser);
            ...
            while ((type = parser.next()) != XmlPullParser.END_DOCUMENT
                ...
                // take care of any preference categories
                if (nodeName.equals(NODE_PREFERENCE_CATEGORY)) {
                    final int categoryDepth = parser.getDepth();
                    final AttributeSet preferenceCategoryAttrs = Xml.asAttributeSet(parser);
                    final String categoryKey = getDataKey(getActivity(), preferenceCategoryAttrs);
                    ...
                    while ((type = parser.next()) != XmlPullParser.END_DOCUMENT
                        ...
                        key = getDataKey(getActivity(), preferenceCategoryAttrs);
                        advanced = getAdvancedBoolean(getActivity(), preferenceCategoryAttrs);

                        if (advanced == null) {
                            // advanced not declared, ignore
                            continue;
                        }

                        if (!advancedModeEnabled && advanced) {
                            // advanced mode is OFF and this preference is advanced
                            preferencesToRemove.add(new PreferenceNodeInfo(categoryKey, key));
                ...
                // take care of the remaining stragglers in the PreferenceScreen
                key = getDataKey(getActivity(), preferenceScreenAttrs);
                advanced = getAdvancedBoolean(getActivity(), preferenceScreenAttrs);

                if (advanced != null && advancedModeEnabled && advanced) {
                    // advanced mode is OFF and this preference is advanced
                    preferencesToRemove.add(new PreferenceNodeInfo(null, key));
                } else if (advanced != null && !advancedModeEnabled && !advanced) {
                    // advanced mode is ON and this preference is not advanced
                    preferencesToRemove.add(new PreferenceNodeInfo(null, key));
                }
            }
