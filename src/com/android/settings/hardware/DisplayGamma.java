...
public class DisplayGamma extends DialogPreference {
    ...
    protected void onBindDialogView(View view) {
        ...
        for (int index = 0; index < mNumberOfControls; index++) {
            ...
            mCurrentColors[index] = Arrays.copyOf(mOriginalColors[index], mOriginalColors[index].length);

            final String defaultKey = "display_gamma_default_" + index;
            if (!prefs.contains(defaultKey)) {
                prefs.edit().putString(defaultKey, Arrays.toString(mOriginalColors[index])).commit();
            }
            ...
            for (int color = 0; color < BAR_COLORS.length; color++) {
                ...
                int min = mCurrentColors[index][CmHardwareManager.GAMMA_CALIBRATION_MIN_INDEX];
                int max = mCurrentColors[index][CmHardwareManager.GAMMA_CALIBRATION_MAX_INDEX];
        ...
        defaultsButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                for (int index = 0; index < mSeekBars.length; index++) {
                    ...
                    final String[] defaultColors = value.replace("[", "")
                        .replace("]", "").split(", ");
        ...
        if (positiveResult) {
            ...
            for (int i = 0; i < mNumberOfControls; i++) {
                editor.putString("display_gamma_" + i,
                    Arrays.toString(mCmHardwareManager.getDisplayGammaCalibration(i)));
            }
            editor.commit();
    ...
    public static void restore(Context context) {
        ...
        for (int i = 0; i < cmHardwareManager.getNumGammaControls(); i++) {
            ...
            if (value != null) {
                final String[] values = value.replace("[", "")
                        .replace("]", "").split(", ");
                rgb[CmHardwareManager.GAMMA_CALIBRATION_RED_INDEX] = Integer.valueOf(values[0]);
                rgb[CmHardwareManager.GAMMA_CALIBRATION_GREEN_INDEX] = Integer.valueOf(values[1]);
                rgb[CmHardwareManager.GAMMA_CALIBRATION_BLUE_INDEX] = Integer.valueOf(values[2]);
    ...
    private class GammaSeekBar implements SeekBar.OnSeekBarChangeListener {
        ...
        public GammaSeekBar(int controlIndex, int colorIndex, ViewGroup container, int min, int max) {
        ...
        public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
            if (fromUser) {
                ...
                mCmHardwareManager.setDisplayGammaCalibration(mControlIndex, mCurrentColors[mControlIndex]);
            }
