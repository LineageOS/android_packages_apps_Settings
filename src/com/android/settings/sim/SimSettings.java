...
public class SimSettings extends RestrictedSettingsFragment implements Indexable {
        ...
        if (isDSDA()) {
            ...
            for (int i = 0; i < mNumSlots; ++i) {
                ...
                if (sir != null && sir.mStatus == SubscriptionManager.ACTIVE) {
                    ...
                    preference.setSummary(sir.displayName + " - " + Integer.toString(i + 1) + " " + sir.number);
            ...
            preference.setTitle(R.string.cellular_data_title);
            category.addPreference(preference);
            preference.setDependency(SIM_DATA_KEY);
    ...
    private boolean isMobileDataEnabled(int id) {
        if (isDSDA()) {
            return android.provider.Settings.Global.getInt(getActivity().getContentResolver(),
                    android.provider.Settings.Global.MOBILE_DATA + id, 0) != 0;
        } else {
    ...
    private boolean isDSDA() {
        TelephonyManager.MultiSimVariants configuration = TelephonyManager.getDefault().getMultiSimConfiguration();
