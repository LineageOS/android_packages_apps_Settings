...
public class SearchPopulator extends IntentService {
            ...
            while ((type=parser.next()) != XmlPullParser.END_DOCUMENT
                ...
                if ("header".equals(nodeName)) {
                    ...
                    int xmlResId = se.getResourceId(
                            com.android.settings.R.styleable.SearchableInfo_xml, 0);
                    ...
                    if (xmlResId != 0) {
                        populateFromXml(xmlResId, header, 1, header.iconRes, header.fragment);
                    }
                } else {
                    XmlUtils.skipCurrentTag(parser);
                }
    ...
    private void populateFromXml(int xmlResId, Header header,
                                 int level, int iconRes, String prefFragment) { // Display settings
        SettingsSearchDatabaseHelper dbHelper = SettingsSearchDatabaseHelper.getInstance(this);
        ...
        try {
            ...
            while ((type = xmlParser.next()) != XmlPullParser.END_DOCUMENT) {
                ...
                if (subXmlId != 0 && !TextUtils.isEmpty(fragment)) {
                    populateFromXml(subXmlId, null, level + 1, header.iconRes, header.fragment);
                    dbHelper.insertEntry(preferenceTitle, level, fragment, header.iconRes);
                } else if (header != null) {
                    header.title = preferenceTitle;
                    header.titleRes = 0;
                    dbHelper.insertHeader(header);
                } else {
                    dbHelper.insertEntry(preferenceTitle, level, prefFragment, iconRes);
                }
            }
    ...
    public static ArrayList<SearchInfo> getTitles(Context ctx) {
        ...
        if (c != null) {
            while (c.moveToNext()) {
                ...
                if (info.fragment != null) System.out.println("Fragment " + info.fragment);
                infos.add(info);
            }
            c.close();
        }
