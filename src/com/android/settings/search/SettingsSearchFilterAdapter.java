...
public class SettingsSearchFilterAdapter extends BaseAdapter implements Filterable {
    ...
    public static class SearchInfo {
        ...
        // used in main thread
        private int mMatchStart = -1, mMatchEnd = -1;
        // updated in filter thread
        private int mPendingMatchStart = -1, mPendingMatchEnd = -1;
    ...
    private ArrayList<SearchInfo> filterInfos(CharSequence constraint) {
        ...
        for (SearchInfo item : mSearchInfo) {
            ...
            item.mPendingMatchStart = -1;
            item.mPendingMatchEnd = -1;
            ...
            if (pos != -1) {
                ...
                for (int ufIndex = pos, fIndex = pos;
                    ...
                    if (fIndex == pos) {
                        item.mPendingMatchStart = ufIndex;
                    }
                    if (fIndex == pos + constraintLen - 1) {
                        ...
                        while (diffLen > constraintLen) {
                            int titleMatchLen = removeNonAlphaNumeric(title.substring(
                                item.mPendingMatchStart, item.mPendingMatchEnd)).trim().length();
                            if (constraintLen < titleMatchLen) {
                                item.mPendingMatchStart +=
                                    (diffLen - titleMatchLen < 1) ? 1 : diffLen - titleMatchLen;
                            } else {
                                break;
                            }
    ...
    private void applyFilteredMatchSpans() {
        for (SearchInfo info : mFilteredInfo) {
            info.mMatchStart = info.mPendingMatchStart;
            info.mMatchEnd = info.mPendingMatchEnd;
        }
    }
