...
public class RootAccessPreferenceController extends DeveloperOptionsPreferenceController
    ...
    private void updatePreference() {
        ...
        CharSequence[] entries = ((ListPreference) mPreference).getEntries();
        boolean entryExists = rootAccess >= 0 && rootAccess < entries.length;

        if (entryExists) {
            ((ListPreference) mPreference).setValueIndex(rootAccess);
        }

        ((ListPreference) mPreference).setSummary(entryExists ? entries[rootAccess] : null);
    }
